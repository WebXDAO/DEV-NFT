...
function ReposList({ reposList, session }) {
  ...
  return (
    <>
      <div className="flex flex-colbg-background">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-6 mx-3">
          {reposList.map((repos) => (
            <div key={repos.id} className="ring-5-primary rounded-xl p-5 shadow-2xl m-2 flex flex-col md:flex-row">
              <a className="text-main" href={repos.clone_url} >
                <div className='mt-5 flex items-center'>
                  <img src={repos.owner.avatar_url} className='rounded-full h-12' />
                  <div className="ml-3">
                    <h2 className="font-semibold"> {repos.name} </h2>
                    <p className="text-text"> {repos.description} </p>
        ...
        <Link href="/">
          <button className="bg-primary text-white font-bold py-2 px-4 m-5 rounded">
            Back home
          </button>
        </Link>
      </div>
